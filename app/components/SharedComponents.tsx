    ...
    <div className="fixed inset-0 z-[9000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in zoom-in-95">
        <div className={`p-8 rounded-2xl w-full max-w-lg shadow-2xl border ${darkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-900'}`}>
            <div className="flex justify-between items-center mb-6 border-b pb-4 border-slate-500/20">
                <h3 className={`text-2xl font-black uppercase italic ${darkMode ? 'text-[#FFC72C]' : 'text-[#522D80]'}`}>{type === 'privacy' ? 'Privacy Policy' : 'About Us'}</h3>
                <button onClick={onClose} className="text-2xl font-bold hover:text-red-500">✕</button>
            </div>
            <p className="font-medium leading-relaxed text-sm opacity-90">{type === 'privacy' ? "Fleetflow Transit Solutions values your privacy. We collect minimal data necessary for internal Gwinnett fleet management, diagnostic tracking, and attendance coordination. We do not sell or share your data with unauthorized third parties. All data is securely handled via industry-standard encrypted databases." : "Fleetflow Transit Solutions is dedicated to modernizing Ride Gwinnett fleet operations. Our management systems provide real-time tracking, inventory analytics, and seamless personnel coordination to keep the transit network moving safely and efficiently. Built for reliability and high visibility on the shop floor."}</p>
            <button onClick={onClose} className="mt-8 w-full py-3 bg-[#522D80] text-white rounded-xl font-black uppercase tracking-widest hover:bg-[#FFC72C] hover:text-[#522D80] transition-colors shadow-lg">Acknowledge & Close</button>
        </div>
    </div>
);
...
export const Footer = ({ onShowLegal, darkMode }: { onShowLegal: (type: 'privacy'|'about')=>void, darkMode?: boolean }) => (
    <div className={`w-full py-6 text-center text-[10px] font-bold tracking-widest uppercase mt-auto border-t ${darkMode ? 'border-slate-800 text-slate-500 bg-slate-900' : 'border-slate-200 text-slate-400 bg-slate-100'}`}>
        <p>© {new Date().getFullYear()} Fleetflow Transit Solutions.</p>
        <div className="flex justify-center gap-6 mt-3">
            <button onClick={() => onShowLegal('privacy')} className={`transition-colors ${darkMode ? 'hover:text-white' : 'hover:text-[#522D80]'}`}>Privacy Policy</button>
            <button onClick={() => onShowLegal('about')} className={`transition-colors ${darkMode ? 'hover:text-white' : 'hover:text-[#522D80]'}`}>About Us</button>
        </div>
    </div>
);
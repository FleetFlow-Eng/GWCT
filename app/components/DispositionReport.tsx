...
export const DispositionReport = ({ showToast, darkMode, isAdmin }: { showToast: any, darkMode: boolean, isAdmin: boolean }) => {
    ...
    const initializeRoutes = async () => {
        if (!confirm("Load default Gwinnett routes?")) return;
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col space-y-6">
            <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-4 mb-2">
                <div>
                    <h2 className={`text-3xl font-black italic uppercase tracking-tighter ${darkMode ? 'text-[#FFC72C]' : 'text-[#522D80]'}`}>Route Disposition</h2>
                    <p className={`text-[10px] font-black uppercase tracking-widest mt-1 ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Gwinnett Place • Fleet Delivery</p>
                </div>
                <div className="flex gap-2">
                    ...
                    {isAdmin && routes.length === 0 && <button onClick={initializeRoutes} className="px-6 py-2.5 bg-[#FFC72C] hover:bg-yellow-500 text-[#522D80] rounded-xl text-[10px] font-black uppercase tracking-widest shadow-md w-full sm:w-auto">Init Routes</button>}
                </div>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 sm:gap-4">
                <div className={`p-4 rounded-2xl border shadow-sm ${bgClass}`}><p className="text-[9px] font-black uppercase opacity-50 mb-1">Total Planned</p><p className="text-2xl sm:text-3xl font-black text-[#522D80] dark:text-[#FFC72C]">{totals.planned}</p></div>
                ...
                <div className={`p-4 rounded-2xl border shadow-sm col-span-2 md:col-span-1 ${bgClass}`}><p className="text-[9px] font-black uppercase opacity-50 mb-1">Delivery %</p><p className={`text-2xl sm:text-3xl font-black ${totals.percent >= 100 ? 'text-green-500' : 'text-[#FFC72C]'}`}>{totals.percent}%</p></div>
            </div>
                    ...
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className={`divide-y ${darkMode ? 'divide-slate-700' : 'divide-slate-100'}`}>
                            {routes.map((r) => {
                                ...
                                return (
                                    <tr key={r.id} onClick={() => { if(isAdmin) { setEditRoute(r); setEditForm({ planned: r.planned, cancelled: r.cancelled, unscheduled: r.unscheduled || 0 }); } }} className={`transition-colors ${isAdmin ? 'cursor-pointer' : ''} ${darkMode ? 'hover:bg-slate-700' : 'hover:bg-purple-50'}`}>
                                        <td className="p-4 font-bold">{r.route}</td><td className="p-4 text-center font-black">{r.planned}</td><td className="p-4 text-center font-black text-red-500">{r.cancelled}</td><td className="p-4 text-center font-black text-blue-500">{r.unscheduled || 0}</td><td className="p-4 text-center font-black text-green-500">{delivered}</td><td className="p-4 text-center"><span className={`px-2.5 py-1 rounded-full text-[9px] font-black uppercase ${percent >= 100 ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>{percent}%</span></td>
                                    </tr>
                                );
                            })}
                        </tbody>
                    ...
                    return (
                        <div key={r.id} onClick={() => { if(isAdmin) { setEditRoute(r); setEditForm({ planned: r.planned, cancelled: r.cancelled, unscheduled: r.unscheduled || 0 }); } }} className={`p-5 rounded-2xl border shadow-sm ${isAdmin ? 'cursor-pointer active:scale-[0.98] transition-transform' : ''} ${bgClass}`}>
                            <div className="flex justify-between items-start mb-4 border-b pb-3 border-slate-500/20">
                                ...
                                <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase ${percent >= 100 ? 'bg-green-500 text-white' : 'bg-amber-500 text-white'}`}>{percent}%</span>
                            </div>
                ...
                <div className="fixed inset-0 z-[6000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in zoom-in-95">
                    <div className={`p-8 rounded-3xl w-full max-w-md shadow-2xl border ${darkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-200 text-black'}`}>
                        <div className="flex justify-between items-center mb-6 border-b pb-4 border-slate-500/20">
                            <h3 className={`text-xl font-black uppercase leading-tight w-5/6 ${darkMode ? 'text-[#FFC72C]' : 'text-[#522D80]'}`}>{editRoute.route}</h3>
                            <button onClick={() => setEditRoute(null)} className="text-2xl font-bold hover:text-red-500">✕</button>
                        </div>
                        <div className="space-y-4 mb-6">
                            <div><label className="text-[10px] font-black uppercase tracking-widest opacity-50 block mb-1">Planned / Required</label><input type="number" className={`w-full p-3.5 rounded-xl font-bold outline-none focus:ring-2 focus:ring-[#FFC72C]/50 ${inputClass}`} value={editForm.planned} onChange={e => setEditForm({...editForm, planned: Number(e.target.value)})} /></div>
                        ...
                        <div className="flex gap-4"><button onClick={() => setEditRoute(null)} className={`w-1/2 py-4 rounded-xl font-black uppercase tracking-widest text-xs transition-colors ${darkMode ? 'bg-slate-700 hover:bg-slate-600 text-white' : 'bg-slate-100 hover:bg-slate-200 text-black'}`}>Cancel</button><button onClick={handleSave} className="w-1/2 py-4 bg-[#522D80] hover:bg-purple-900 text-white rounded-xl font-black uppercase tracking-widest text-xs shadow-lg transition-colors">Save Route</button></div>
                    </div>
                </div>
            )}
        </div>
